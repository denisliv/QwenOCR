                    ...
                    if last_user_msg_index is not None:
                        ...
                        if isinstance(original_content, str):
                            ...
                            if content_normalized.startswith("### Task:") and "</context>" in content_normalized:
                                ...
                                logger.info(f"Cleaned OpenWebUI preamble. User query: {user_text!r}")
                            else:
                                user_text = original_content.strip()
                                logger.debug(f"No OpenWebUI preamble detected. Keeping original content: {user_text!r}")
                        ...
                        if file_names:
                            ...
                            new_content.append({"type": "text", "text": file_names_text})
                        ...
                        file_names = [f["name"] for f in self._new_files]
                        file_names_text = "\n".join([f"Имя файла: {name}" for name in file_names]) if file_names else ""
                        content = []
                        if file_names_text:
